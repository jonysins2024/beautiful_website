...
const Control_Post = () => {
  ...
  const Make_Hide = (user) => {
    fetch(`https://backend-kappa-puce.vercel.app/user_data/Approved_or_Deny/${user._id}`, {
      method: "PATCH",
          ...
          <tbody>
            {Boookings.map(User => (
              <tr key={User._id}>
                ...
                {
                  User.Status === "Denied" && <td className='py-3' data-label="Make Admin">{
                    User.Status === 'Approved' ? <p className={`bg-red-200 clipo9 cursor-not-allowed text-center py-3 w-[50px] mx-auto rounded-lg`}><button className='cursor-not-allowed text-center'>Approved</button></p> : <p onClick={() => Make_Hide(User)}><p onClick={handleClick0} className={`${isScaled0 ? 'scale-105' : ''} duration-300 clipo9 bg-lime-500 text-center cursor-pointer py-3 w-[150px] mx-auto rounded-lg`}><button>Approve User</button></p></p>
                  }</td>
                }
